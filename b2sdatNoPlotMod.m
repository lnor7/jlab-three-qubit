...
phc0 = str2num(nmrx('getparm f phc0'));
phc1 = str2num(nmrx('getparm f phc1'));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% get phase calibration from apk
nmrx('acqu')
nmrx('ft')
nmrx('ft')
nmrx('apk')

%alt_spect = nmrx('getdat');
%alt_spect = alt_spect.';

phc0 = phc0 + str2num(nmrx('getparm f phc0'));
phc1 = phc1 + str2num(nmrx('getparm f phc1'));
...
for k = 1:nexpt
  spect(:,k) = dophase(spect(:,k),fdat,[sfo phc0 phc1],sfo,swh);
end
...
for k = 1:size(phase,1)	% loop over frequencies
  if abs(phase(k,1)-sfo)<1e6
    p1 = 47.75 +180;% +180 is just a convenient convention
    p2 = phase(k,3);

    % rephase spectrum using phases we found
    pv = (p1 + (fdat-sfo) * p2)*pi/180;
    y = x .* exp(i * pv);
    return;
  end
end
